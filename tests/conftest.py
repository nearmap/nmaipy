...
@pytest.fixture(scope="session")
def features_2_all_packs_df(data_directory: Path) -> pd.DataFrame:
    """
    Combined Feature API (all packs) + Roof Age API data for 100 NJ parcels.
    Generated by test_gen_data_2_all_packs in test_parcels.py.
    Returns a plain DataFrame (not GeoDataFrame) since geometry parsing is
    expensive and the whitelist tests only need class_id.
    """
    filepath = data_directory / "test_features_2_all_packs.csv"
    if not filepath.exists():
        pytest.skip("test_features_2_all_packs.csv not generated yet — run test_gen_data_2_all_packs first")
