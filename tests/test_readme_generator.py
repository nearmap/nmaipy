...
class TestRoofAgeDatasetSection:
    ...
    def _write_config(self, tmp_path, **params):
        (tmp_path / "export_config.json").write_text(json.dumps({"_metadata": {}, "parameters": params}))
...
class TestColumnMetadataTables:
    ...
    def test_column_dicts_are_columnmeta_instances(self):
        ...
        for label, columns in (
            ...
            ("ADDRESS_QUERY_COLUMNS", ADDRESS_QUERY_COLUMNS),
            ("RSI_COLUMNS", RSI_COLUMNS),
            ("ROOF_AGE_COLUMNS", ROOF_AGE_COLUMNS),
            ("DOMINANT_ROOF_MATERIAL_COLUMNS", DOMINANT_ROOF_MATERIAL_COLUMNS),
            ("DOMINANT_ROOF_TYPES_COLUMNS", DOMINANT_ROOF_TYPES_COLUMNS),
    ...
    def test_confidence_columns_dtype_marks_quantised_uint8(self):
