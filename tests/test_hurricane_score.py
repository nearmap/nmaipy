...
def roof_with_both_scores(hurricane_score_payload):
    ...
    for feature in hurricane_score_payload["features"]:
        if feature.get("classId") == ROOF_ID and "hurricaneScore" in feature and "roofSpotlightIndex" in feature:
            return feature
...
def all_roofs_with_hurricane_score(hurricane_score_payload):
    ...
    roofs = [f for f in hurricane_score_payload["features"] if f.get("classId") == ROOF_ID and "hurricaneScore" in f]
    if not roofs:
...
def test_hurricane_score_with_real_data(roof_with_hurricane_score):
    ...
    assert (
        result["hurricane_vulnerability_probability"]
        == roof_with_hurricane_score["hurricaneScore"]["vulnerabilityProbability"]
    )

    assert "hurricane_vulnerability_rate_factor" in result
    assert (
        result["hurricane_vulnerability_rate_factor"]
...
def test_hurricane_score_different_values(all_roofs_with_hurricane_score):
    ...
    roof_feature = (
        all_roofs_with_hurricane_score[1]
        if len(all_roofs_with_hurricane_score) > 1
        else all_roofs_with_hurricane_score[0]
    )
...
def test_hurricane_score_and_roof_spotlight_index_together(roof_with_both_scores):
    ...
    assert (
        result["hurricane_vulnerability_probability"]
        == roof_with_both_scores["hurricaneScore"]["vulnerabilityProbability"]
    )
    assert (
        result["hurricane_vulnerability_rate_factor"]
        == roof_with_both_scores["hurricaneScore"]["vulnerabilityRateFactor"]
    )
